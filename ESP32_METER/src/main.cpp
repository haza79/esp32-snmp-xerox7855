...
int retryDelayMs = 60000;  // delay between retries
...
String setCommandUrl;
String pingUrl;

void setupDomain(){
  domain = String(config.hostname);
  addEspUrl = domain+"/addEsp.php";
  getEspIdUrl = domain+"/getPrinterIdByEspSerial.php";
  addMeterUrl = domain+"/addMeter.php";
  getCommandUrl = domain+"/getCommand.php";
  setCommandUrl = domain+"/setCommand.php";
  pingUrl = domain+"/ping.php";
}
...
String pingServer();
bool waitForSNMPResponse(unsigned long timeout);
...
server.on("/", []() {
    ...
    page += "<h2>ESP Serial: "+getChipID()+"</h2>";
    page += "<h2>Wifi SSID: "+String(config.ssid)+"</h2>";
    page += "<h2>Wifi PASS: "+String(config.password)+"</h2>";
    page += "<h2>ESP32 WiFi Setup</h2>";
    page += "<h3>Select WiFi Network</h3>";

    page += "<ul>";
    for (int i = 0; i < n; i++) {
        page += "<li><a href='/select?ssid=";
        page += WiFi.SSID(i);
        page += "'>";
        page += WiFi.SSID(i);
        page += " (";
...
server.on("/select", []() {
    ...
    page += "<h2>Connect to: " + ssid + "</h2>";
    page += "<form action='/test' method='POST'>";
    page += "<input type='hidden' name='ssid' value='" + ssid + "'>";
    page += "Host name: <input name='hostname' value='"+String(config.hostname)+"'><br>";
    page += "ESP 32 Board name: <input name='esp32Name' value='"+String(config.esp32Name)+"'><br>";
    page += "Password: <input name='password' type='password'><br><br>";
    page += "Printer IP: <input name='printer' value='"+String(config.printerIP)+"'><br><br>";
    page += "<input type='submit' value='Test Connection'>";
    page += "</form></body></html>";
...
server.on("/test", []() {
    ...
    String esp32name = server.arg("esp32Name");
    String password = server.arg("password");
    String printer = server.arg("printer");

    WiFi.begin(ssid.c_str(), password.c_str());

    int timeout = 0;
    while (WiFi.status() != WL_CONNECTED && timeout < 20) { // 10 seconds
        delay(500);
        timeout++;
    }
    ...
    if (WiFi.status() == WL_CONNECTED) {
        ...
        prefs.putString("esp32Name", esp32name);
        prefs.putString("ssid", ssid);
        prefs.putString("pass", password);
        prefs.putString("printer", printer);
        prefs.putBool("firstSetup", true);
        prefs.end();

        String page = "<html><body><h2>Connected Successfully!</h2>";
        page += "<p>Saving config & rebooting...</p>";
        page += "</body></html>";
        server.send(200, "text/html", page);
        ...
        String page = "<html><body><h2>❌ Failed to connect to WiFi</h2>";
        page += "<p>Please try again.</p>";
        page += "<a href='/'>Back</a>";
        page += "</body></html>";
        server.send(200, "text/html", page);
        digitalWrite(LED,LOW);
